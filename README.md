...
> A semantic discovery engine that understands the "soul" of cinema. Moving beyond keyword-matching, it uses a 3-stage RAG pipeline and BGE embeddings to provide contextual recommendations across a 95,000+ film corpus. From "mind-bending loops" to "atmospheric noir," it delivers hyper-curated results with natural language justifications.
...
* **Capability:** Built an agentic orchestration layer via **LangGraph** to handle real-time metadata filtering (streaming platforms, Oscars) and dynamic OOV (Out-Of-Vocabulary) handling for new releases in the filmdb app.
